...
class Bridge:
    ...
    def connect_node_to_beam(self, node, beam):
        """ Snaps an existing node onto a beam, splitting the beam. """
        if node == beam.node_a or node == beam.node_b:
            return
        ...
        # Project node onto beam line
        t = ((node.x - x1) * dx + (node.y - y1) * dy) / (dx*dx + dy*dy)
        t = max(0.0, min(1.0, t)) 
        
        # Move node to exact position on beam
        node.x = x1 + t * dx
        node.y = y1 + t * dy
